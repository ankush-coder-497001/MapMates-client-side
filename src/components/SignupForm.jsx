...
import { Register } from "../services/user.svc"
...
export default function SignupForm({ onSwitchToLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.password.length < 6) {
      toast.error("Password must be at least 6 letters long !");
      return;
    }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Call your Register function with the user data
      ...
      <div className="w-full max-w-md mx-auto">
        <div className="backdrop-blur-md bg-white/6 border border-white/10 rounded-3xl p-6 shadow-2xl shadow-black/20">
          ...
          <div className="text-center mb-6">
            <h2 className="text-2xl font-bold text-gray-100 mb-1">Join ChatGlass</h2>
            <p className="text-gray-400 text-sm">Create your account and start connecting</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-start space-x-2">
              <input
                ...
                id="terms"
                required
                className="w-4 h-4 mt-1 rounded border-gray-600 bg-white/6 text-[#E50914] focus:ring-[#E50914]/50 focus:ring-2"
              />
              <label htmlFor="terms" className="text-xs text-gray-300 leading-relaxed">
                I agree to the{" "}
                <button type="button" className="text-[#E50914] hover:text-rose-400 transition-colors duration-200">
                  Terms of Service
                </button>{" "}
                and{" "}
                <button type="button" className="text-[#E50914] hover:text-rose-400 transition-colors duration-200">
